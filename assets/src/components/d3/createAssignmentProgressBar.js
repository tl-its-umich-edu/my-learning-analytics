...
function createAssignmentProgressBar ({
  data,
  width,
  height,
  domElement,
  ...
  bar.append('rect')
    .attr('x', _ => x(0))
    .attr('width', x(currentGrade))
    .attr('y', margin.top)
    .attr('height', y.bandwidth())
    .attr('fill', '#e1e1e1')

  bar.append('rect')
    .attr('x', _ => x(currentGrade))
    .attr('width', x(maxGrade) - x(currentGrade))
    .attr('y', margin.top)
    .attr('height', y.bandwidth())
    .attr('fill', '#e1e1e1')
...
export default createAssignmentProgressBar
