...
function createHistogram ({ data, width, height, domElement, xAxisLabel, yAxisLabel, myGrade, maxGrade = 100,
  ...
  const myGradePosition = (myGrade)=>{
    // mygrades is in between binned and normal distribution
  ...
  // Default's to not show count on bar text, in case of AB testing we want to leave it a configurable option
  if (showNumberOnBars) {
    bar.append('text')
  ...
  console.log(firstTick._groups[0][0].innerHTML)
    ...
    if (myGrade) {
      svg.append('line')
        .attr(`transform`, `translate(0, ${aHeight - margin.bottom})`)
        .attr('x1', x(myGradePosition(myGrade)))
        .attr('y1', -aHeight+75)
        .attr('x2', x(myGradePosition(myGrade)))
        .attr('y2', 0)
        .attr('stroke', 'darkorange')
        .attr('stroke-width', '2')
      svg.append('text')
        .attr('x', x(myGradePosition(myGrade)))
