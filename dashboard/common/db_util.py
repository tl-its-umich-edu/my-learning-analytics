...
def get_course_view_options (course_id):
    ...
    if (course_id):
        with django.db.connection.cursor() as cursor:
            cursor.execute("SELECT show_resources_accessed, show_assignment_planning, show_assignment_planning_v2, show_grade_distribution FROM course_view_option WHERE course_id = %s", [course_id])
            row = cursor.fetchone()
            if (row != None):
                course_view_option = {}
                ...
                course_view_option['show_assignment_planning_v2'] = row[1] and 'show_assignment_planning_v2' not in settings.VIEWS_DISABLED
