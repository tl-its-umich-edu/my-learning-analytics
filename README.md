...
1. Go to http://localhost:5001/courses/**_`canvas_id`_** with the `canvas_id` you found. (For example, with SQL file `myla_test_data_2019_10_16.sql` loaded, Go to http://localhost:5001/courses/235420 or http://localhost:5001/courses/362855 and view the course page as an admin.)
...
**_(Django Cron) Run only at 2AM_**

RUN_AT_TIMES=2:00

**_(Unix Cron) - Run every 5 minutes_**

CRONTAB_SCHEDULE=_/5 _ \* \* \*
...
1. copy env.json file from env_sample.json file, and add configuration values;
   ```sh
   cp scripts/data_validation/env_sample.json scripts/data_validation/env.json
   ```
2. run data validation script within docker container:

   ```sh
   docker exec -it student_dashboard /bin/bash -c "python scripts/data_validation/<script file>"
   ```
