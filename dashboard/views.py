...
from dashboard.common.db_util import canvas_id_to_incremented_id, \
    get_user_courses_info
from dashboard.common import utils
from django.core.exceptions import ObjectDoesNotExist
from collections import namedtuple

from dashboard.models import Course, CourseViewOption, Resource, \
...
def get_course_template(request: HttpRequest, course_id: 0) -> HttpResponse:
...
def resource_access_within_week(request: HttpRequest,
    ...
    eventlog(request.user, EventLogTypes.EVENT_VIEW_RESOURCE_ACCESS.value,
             extra=data)
    ...
    logger.debug("course_start=" + str(course_date_start) + " start=" + str(
        start) + " end=" + str(end))
    ...
    output_df = pd.DataFrame(0.0, index=resource_id_name,
                             columns=[GRADE_A, GRADE_B, GRADE_C, GRADE_LOW,
    ...
    output_df['resource_id_part'], output_df['resource_name_part'] = output_df[
        'resource_id_name'].str.split(';', 1).str

    output_df['resource_name'] = output_df.apply(
        lambda row: RESOURCE_URLS[row.resource_type][
                        "prefix"] + row.resource_id_part +
                    RESOURCE_URLS[row.resource_type][
                        "postfix"] + CANVAS_FILE_ID_NAME_SEPARATOR + row.resource_name_part,
        axis=1)
    output_df.drop(
...
def grade_distribution(request: HttpRequest, course_id: 0) -> HttpResponse:
    ...
    (select current_grade from user where sis_name=%(current_user)s and course_id=%(course_id)s) as current_user_grade
        from user where course_id=%(course_id)s and enrollment_type='StudentEnrollment';
                    """
    df = pd.read_sql(grade_score_sql, conn,
                     params={"current_user": current_user,
                             'course_id': course_id})
    ...
    if BinningGrade is not None and not BinningGrade.binning_all:
        df['current_grade'] = df['current_grade'].replace(
            df['current_grade'].head(BinningGrade.index),
            BinningGrade.value)
    ...
    if df[df['current_grade'] > 100.0].shape[0] > 0:
        ...
        df['current_grade'] = df['current_grade'].apply(
            lambda x: 99.99 if x == 100.00 else x)
    ...
    eventlog(request.user, EventLogTypes.EVENT_VIEW_GRADE_DISTRIBUTION.value,
             extra=data)
...
@permission_required('dashboard.update_user_default_selection_for_views',
                     fn=objectgetter(Course, 'course_id', 'canvas_id'),
                     raise_exception=True)
def update_user_default_selection_for_views(request: HttpRequest,
                                            course_id: 0) -> HttpResponse:
    ...
    eventlog(request.user, EventLogTypes.EVENT_VIEW_SET_DEFAULT.value,
             extra=data)
    key = 'default'
    try:
        obj, create_or_update_bool = UserDefaultSelection.objects.set_user_defaults(
            int(course_id), current_user,
            default_type,
            default_type_value)
        logger.info(
...
def is_weight_considered(course_id: int) -> bool:
...
def courses_enabled(request: HttpRequest) -> Union[HttpResponse, HttpResponseForbidden]:
