...
var makeGraph = function(url) {
    ...
    $.getJSON(url || "/"+dashboard.course_id+"/file_access_within_week?week_num_start=1&week_num_end=0", function (initResult) {
...
function makeGrapBasedOnGradeAndSlide(grade, silderValues)
{
    ...
    makeGraph('/'+dashboard.course_id+'/file_access_within_week?week_num_start=' + startWeek + "&week_num_end=" + endWeek + "&grade=" + grade);
}
...
makeSlider = function () {
    ...
    $.getJSON("/"+dashboard.course_id+"/get_current_week_number", function (initResult) {
        if (initResult.length === 0) {
            // return no data
            return "no data";
        }
